...
pygame.display.set_caption('Night Pool with Colorful Rain')
...
# load picture
bg = pygame.image.load("../graphics/background/background.png").convert_alpha()

position = bg.get_rect()
position.center = screen_width // 2, screen_height // 2
...
while run:
    ...
    # Draw background
    screen.fill(BLACK)
    blit_alpha(screen, bg, position, 255)
    ...
    if keys[pygame.K_SPACE] and (flash == False):
        ...
        thunder_sound = pygame.mixer.Sound('../node/deco/thunder.wav')
        thunder_sound.play()
    ...
    # # bg music
    # pygame.mixer.music.load('../node/bg/rain.wav')
